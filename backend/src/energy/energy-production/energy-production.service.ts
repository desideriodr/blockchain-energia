...
export class EnergyProductionService {
  ...
  /** getProductionDashboard - retorna datos de produccion para el dashboard */
  async getProductionDashboard(user: User, productionBySourceLoader?: any) {
    ...
    if (productionBySourceLoader && sources.length > 0) {
      // Con DataLoader: 2 queries totales sin importar cuántas fuentes
      const metrics = await Promise.all(
        sources.map(source => productionBySourceLoader.load(source.id))
      );
      ...
      sources.forEach((source, i) => {
        result.push({
          ...
          producedTotal: metrics[i].producedTotal,
          producedToday: metrics[i].producedToday,
        });
      });
    } else {
      // Fallback sin loader (tests, contextos sin GraphQL)
  ...
  /* getEnergyProducedByDay - retorna energia producida por dia  */
